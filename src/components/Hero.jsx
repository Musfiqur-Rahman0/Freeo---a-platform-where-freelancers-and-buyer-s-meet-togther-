...
const Hero = () => {
  ...
  return (
    <div className="relative w-full ">
      ...
      <video
        src={herovideo}
        className="w-full h-[680px] z-0 object-cover"
        autoPlay
        muted
        loop
      ></video>
      <div className="absolute inset-0 h-full w-full  z-10 pt-[176px] pb-8">
        <div className="max-w-7xl  h-full mx-auto flex flex-col justify-between">
          <div className="w-4/6 space-y-5">
            <h2 className="text-white text-5xl font-bold max-w-[610px]">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {categories.map((category, i) => (
                <div
                  key={i}
                  className="cursor-pointer text-white font-semibold capitalize px-4 py-2 rounded-lg bg-transparent border border-white"
                >
                  {category}
                </div>
              ))}
